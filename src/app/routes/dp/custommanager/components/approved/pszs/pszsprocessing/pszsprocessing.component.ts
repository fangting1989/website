...
import {ApprovedPszsComponent} from '../approved-pszs/approved-pszs.component';
import {HhrAchivedComponent} from './../../common/hhr-achived/hhr-achived.component';

@Component({
  selector: 'app-pszsprocessing',
  templateUrl: './pszsprocessing.component.html',
  styleUrls: ['./pszsprocessing.component.scss']
})
export class PszsprocessingComponent implements OnInit {
  DataList:any = []
  searchObject:any = {};
  PageNum:any = 1;
  PageSize:any = 10;
  ...
  editItem(item){
    ...
    const modal = this.modalHelper.create(ApprovedPszsComponent,{ data: data},{size:800}).subscribe(res => {
      this.loadData()
    });
  }
}
