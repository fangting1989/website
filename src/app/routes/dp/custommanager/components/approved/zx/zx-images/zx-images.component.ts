...
@Component({
  selector: 'app-zx-images',
  templateUrl: './zx-images.component.html',
  styleUrls: ['./zx-images.component.scss']
})
export class ZxImagesComponent implements OnInit {
    ...
    if (this.Images.length > 0) {
      ...
      _.each(this.Images, function (obj) {
        if (typeof obj.content != 'undefined') {
          ...
          formData.append("tablename", "zximage");
          formData.append("filepath","zximage")
  ...
  saveSubmitClick(){
    ...
    this.modalService.confirm({
      nzTitle     : '提示',
      nzContent   : '<b style="color: red;">是否确定已经保存图片并提交到下一环节?</b>',
      nzOkText    : '确定',
      nzOkType    : 'danger',
      nzOnOk      : () => {
        var postData = {
        ...
        self.dataServices.tpartnerdsksh(postData).subscribe(result => {
          if (result != null) {
