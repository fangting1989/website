...
@Component({
  selector: 'app-hhr-edit',
  templateUrl: './hhr-edit.component.html',
  styleUrls: ['./hhr-edit.component.scss']
})
export class HhrEditComponent implements OnInit {
  data: any;
  model: any = {}
  seladdress:any;
  EnterPriseCode: any;
  ...
  saveClick(state) {
    ...
    //判断条件
    if(Utils.IsStringNullOrEmpty(this.model.partnername)){
      this.msg.error("请填写负责人信息")
      return;
    }
  ...
  //   const modal = this.modalHelper.create(HhrEditComponent,{ data: data},{size:500}).subscribe(res => {
