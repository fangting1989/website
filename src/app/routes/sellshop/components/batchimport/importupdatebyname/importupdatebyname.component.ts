...
@Component({
  selector: 'app-importupdatebyname',
  templateUrl: './importupdatebyname.component.html',
  styleUrls: ['./importupdatebyname.component.scss']
})
export class ImportupdatebynameComponent implements OnInit {
  ...
  uploadImg() {
    ...
    if (this.model.FileObject) {
      self.title_desc = ""
      this.dataServices.importupdatebyname_check(formData).subscribe(result => {
        if(result){
  ...
  ImportClick(event){
    ...
    this.dataServices.importupdatebyname(postData).subscribe(result => {
      if(result){
  ...
  downloadtemp(event){
    var self = this;
    self.dataServices.download_file({filename:'import_updateproductbyname_template.xlsx'}).subscribe(result => { 
      if(result){
