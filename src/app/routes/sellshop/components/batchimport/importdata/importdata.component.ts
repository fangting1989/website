...
export class ImportdataComponent implements OnInit {
  ...
  uploadImg() {
    ...
    if (this.model.FileObject) {
      self.title_desc = ""
      this.dataServices.importdata_check(formData).subscribe(result => {
        if(result){
  ...
  ImportClick(event){
    ...
    this.dataServices.importdata(postData).subscribe(result => {
      if(result){
  ...
  downloadtemp(event){
    var self = this;
    self.dataServices.download_file({filename:'import_product_template.xlsx'}).subscribe(result => { 
      if(result){
